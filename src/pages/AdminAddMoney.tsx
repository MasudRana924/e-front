...
type AdminAddMoneyFormData = z.infer<typeof adminAddMoneySchema>

const AdminAddMoney = () => {
  const navigate = useNavigate()
  const { user } = useAuth()
  const { adminAddMoney, adminLoading, adminError, clearAdminError } = useAdmin()
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<AdminAddMoneyFormData>({
    resolver: zodResolver(adminAddMoneySchema),
  })
  ...
  const onSubmit = async (data: AdminAddMoneyFormData) => {
    try {
      const result = await adminAddMoney(data).unwrap()
      toast.success(result.message || 'Money added successfully', {
        description: result.data?.balance ? `New balance: ৳${result.data.balance.toLocaleString()}` : undefined,
      })
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      ...
      <div className="mb-6 mt-6 text-center">
        ...
        <p className="text-muted-foreground mt-4 mb-4 text-left">Add money to your super admin account</p>
      </div>
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        ...
        >
          {adminLoading ? "Processing..." : "Add Money"}
        </Button>
      </form>
      </div>
    </div>
